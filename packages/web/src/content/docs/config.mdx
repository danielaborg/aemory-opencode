...
The imported JSON is substituted directly into your config before parsing, allowing you to extract complex objects like provider configurations into separate files.

File paths follow the same rules as `{file:...}`:

- Relative to the config file directory
- Absolute paths starting with `/` or `~`

Imported files can also contain their own `{import:...}`, `{file:...}`, and `{env:...}` references. Circular imports are detected and will throw an error.
